...
import { Trophy, Percent, Users } from "lucide-react";
...
function HomePage() {
  ...
  useEffect(() => {
    supabase.from("player_stats").select("*").then(({ data }) => {
      if (data) setPlayerStats(data as PlayerStat[]);
    });

    supabase.from("partner_filtered_ranking").select("*").eq("partner_name", "Mihir").then(({ data }) => {
      if (data) {
        const rankings = data as PartnerRanking[];
        setMihirStats(rankings);
        const ids = rankings.map((r) => r.player_id).filter(Boolean) as string[];
        if (ids.length) {
          supabase.from("players").select("id, avatar_url").in("id", ids).then(({ data: players }) => {
            if (players) {
  ...
  const statsByAmp = [...playerStats].sort((a, b) => (b.avg_match_points ?? 0) - (a.avg_match_points ?? 0));
  const statsByWin = [...playerStats].sort((a, b) => (b.win_pct ?? 0) - (a.win_pct ?? 0));
  const mihirSorted = [...mihirStats].sort((a, b) => (b.wins_together ?? 0) - (a.wins_together ?? 0));
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto max-w-5xl px-4 py-12 flex flex-col items-center text-center gap-6">
        ...
        <p className="text-muted-foreground max-w-xl">
          ...
          of an athlete just seeing your name here even when you play absolutely Ass.
        </p>
        <Link
          to="/matches"
      ...
      <section className="mx-auto max-w-5xl px-4 pb-16">
        ...
        <Tabs defaultValue="amp">
          <TabsList className="mb-4">
            ...
            <TabsTrigger value="mihir" className="gap-1.5"><Users className="w-4 h-4" /> Wins with Mihir</TabsTrigger>
          </TabsList>
                ...
                <TableBody>
                  {mihirSorted.map((p) => (
                    <TableRow key={p.player_id}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <PlayerAvatar url={avatarMap[p.player_id ?? ""] ?? null} name={p.player_name} />
                      ...
                      <TableCell className="text-right font-semibold">
                        {p.win_pct_together != null ? `${Number(p.win_pct_together).toFixed(0)}%` : "—"}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
